...
const addPlantImage = (event) => {
  ...
  image.setAttribute(
    "src",
    plantImage ? plantImage : "images/stock-plant.jpeg"
  );
...
const displayWaterData = (event) => {
  ...
  const waterDisplay = document.getElementById(`water-details-${buttonIndex}`);
  if (waterDisplay) {
    waterDisplay.remove();
  ...
  let waterInfoDiv = document.createElement("div");
  let waterInfo = document.createElement("p");
  waterInfo.setAttribute("class", "water-details");
  waterInfo.setAttribute("id", `water-details-${buttonIndex}`);
  waterInfo.innerHTML = waterData;

  waterInfoDiv.appendChild(waterInfo);
  let imageCard = plantBoxObj.element.children[0];

  imageCard.appendChild(waterInfoDiv);
    ...
    for (let i = 0; i < Math.ceil(storedPlants.length / 5); i++) {
      ...
      for (j = i * 5; j < i * 5 + 5; j++) {
        ...
        if (box.children[0]) {
          ...
          box.children[0].children[3].addEventListener(
            "click",
            displayWaterData
          );
        }
